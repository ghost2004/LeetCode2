        ...
        for (int i = 1; i < prices.length; i++) {
            if (prices[i] < prices[i-1]) {
                if (curMin < prices[i-1])
                    max += prices[i-1] - curMin;
                curMin = prices[i];
            } 
        }
        
        if (prices[prices.length-1] > curMin)
            max += prices[prices.length-1] - curMin;
